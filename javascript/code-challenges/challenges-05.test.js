...
const toLastNames = people => {
  let wholeName = people.map(person => `${person.firstName} ${person.lastName}`);
  return wholeName;
};
...
const addPurchases = (arr) => {
  let price = arr.reduce((acc, val, idx) => {
...
const reversedString = (str) => {
  let revWords = str.split("").reduce((rev, char) => {
    rev = char + rev;
    return rev;
  }, []);
  return revWords;
